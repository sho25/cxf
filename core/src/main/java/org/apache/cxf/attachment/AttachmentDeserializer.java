...
condition|)
block|{
comment|// this gets handled using the logic defined by
comment|// the addHeaderLine method.  If this line is a continuation, but
comment|// there's nothing before it, just call addHeaderLine to add it
comment|// to the last header in the headers list
if|if
condition|(
name|buffer
operator|.
...
return|return;
block|}
comment|// we need to test the first character to see if this is a continuation whitespace
name|char
name|ch
init|=
name|line
...
argument_list|)
decl_stmt|;
comment|// tabs and spaces are special.  This is a continuation of the last header in the list.
if|if
condition|(
name|ch
operator|==
...
argument_list|()
decl_stmt|;
comment|// it's possible that we have a leading blank line.
if|if
condition|(
name|size
operator|>
...
block|{
comment|// this just gets appended to the end, preserving the addition order.
name|headers
operator|.
name|add
argument_list|(
