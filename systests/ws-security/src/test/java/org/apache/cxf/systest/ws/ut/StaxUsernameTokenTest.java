...
expr_stmt|;
block|}
comment|/*         // TODO         // Streaming         ((BindingProvider)utPort).getRequestContext().put(             SecurityConstants.ENABLE_STREAMING_SECURITY, "true"         );         ((BindingProvider)utPort).getResponseContext().put(             SecurityConstants.ENABLE_STREAMING_SECURITY, "true"         );         try {             utPort.doubleIt(25);             fail("Failure expected on no UsernameToken");         } catch (javax.xml.ws.soap.SOAPFaultException ex) {             String error = "UsernameToken not satisfied";             assertTrue(ex.getMessage().contains(error));         }         */
operator|(
operator|(
name|java
operator|.
