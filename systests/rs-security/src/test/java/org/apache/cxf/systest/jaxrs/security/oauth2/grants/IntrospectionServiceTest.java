...
begin_comment
comment|/**  * Some unit tests for the token introspection service in CXF. The tests are run multiple times with different  * OAuthDataProvider implementations:  * a) PORT - EhCache  * b) JWT_PORT - EhCache with useJwtFormatForAccessTokens enabled  * c) JCACHE_PORT - JCache  * d) JWT_JCACHE_PORT - JCache with useJwtFormatForAccessTokens enabled  * e) JPA_PORT - JPA provider  * f) JWT_NON_PERSIST_JCACHE_PORT-  JCache with useJwtFormatForAccessTokens + !persistJwtEncoding  */
end_comment
...
name|getResource
argument_list|(
literal|"introspection-server.xml"
argument_list|)
decl_stmt|;
specifier|protected
name|void
...
name|getResource
argument_list|(
literal|"introspection-server-jwt.xml"
argument_list|)
decl_stmt|;
specifier|protected
name|void
