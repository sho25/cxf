...
begin_comment
comment|/**  * Some unit tests for the UserInfo Service in OpenId Connect. This can be used to return the User's claims given  * an access token. The tests are run multiple times with different OAuthDataProvider implementations:  * a) PORT - EhCache  * b) JWT_PORT - EhCache with useJwtFormatForAccessTokens enabled  * c) JCACHE_PORT - JCache  * d) JWT_JCACHE_PORT - JCache with useJwtFormatForAccessTokens enabled  * e) JPA_PORT - JPA provider  * f) JWT_NON_PERSIST_JCACHE_PORT-  JCache with useJwtFormatForAccessTokens + !persistJwtEncoding  */
end_comment
