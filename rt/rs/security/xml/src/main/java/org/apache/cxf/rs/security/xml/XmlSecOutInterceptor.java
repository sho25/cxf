...
name|phase
decl_stmt|;
comment|//private boolean encryptSymmetricKey = true;
comment|//private SecretKey symmetricKey;
specifier|private
name|boolean
name|signRequest
decl_stmt|;
...
condition|)
block|{
comment|// Configure algorithms
comment|/*                 if (encryptionProperties != null) {                     if (encryptionProperties.getEncryptionKeyTransportAlgo() != null) {                         properties.setEncryptionKeyTransportAlgorithm(                             encryptionProperties.getEncryptionKeyTransportAlgo());                     }                     if (encryptionProperties.getEncryptionSymmetricKeyAlgo() != null) {                         properties.setEncryptionSymAlgorithm(                             encryptionProperties.getEncryptionSymmetricKeyAlgo());                     }                     if (encryptionProperties.getEncryptionDigestAlgo() != null) {                         properties.setEncryptionKeyTransportDigestAlgorithm(                             encryptionProperties.getEncryptionDigestAlgo());                     }                     */
comment|/* TODO                     if (encryptionProperties.getEncryptionKeyIdType() != null) {                         if (encryptionProperties.getEncryptionKeyIdType().equals(SecurityUtils.X509_CERT)) {                         } else if (encryptionProperties.getEncryptionKeyIdType().equals(                             SecurityUtils.X509_ISSUER_SERIAL)) {                                                      } else {                             throw new Exception("Unsupported key identifier:" + keyIdType);                         }                     }                 }                 */
block|}
if|if
condition|(
name|signRequest
...
expr_stmt|;
block|}
comment|/*     private void configureKeys(Message message, XMLSecurityProperties properties)          throws Exception {         if (symmetricKey != null) {             properties.setEncryptionKey(symmetricKey);         }         if (encryptSymmetricKey) {                      }         String userName =              (String)message.getContextualProperty(SecurityConstants.ENCRYPT_USERNAME);         CryptoLoader loader = new CryptoLoader();         Crypto crypto = loader.getCrypto(message,                                    SecurityConstants.ENCRYPT_CRYPTO,                                   SecurityConstants.ENCRYPT_PROPERTIES);                  userName = SecurityUtils.getUserName(crypto, userName);         if (StringUtils.isEmpty(userName)) {             throw new Exception("User name is not available");         }         X509Certificate receiverCert = getReceiverCertificateFromCrypto(crypto, userName);         if (receiverCert == null) {             throw new Exception("Receiver certificate is not available");         }                  properties.setEncryptionKey(receiverCert.getPublicKey());         properties.setEncryptionUseThisCertificate(receiverCert);     }          private X509Certificate getReceiverCertificateFromCrypto(Crypto crypto, String user) throws Exception {         X509Certificate[] certs = SecurityUtils.getCertificates(crypto, user);         return certs[0];     }     */
specifier|private
name|void
name|configureSignature
parameter_list|(
