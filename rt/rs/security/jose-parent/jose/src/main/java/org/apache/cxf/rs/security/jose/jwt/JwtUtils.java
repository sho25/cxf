...
parameter_list|)
block|{
comment|// If the expected audience is configured, a matching "aud" must be present
name|String
...
throw|;
block|}
comment|// Otherwise if we have no aud claims then the token is valid
if|if
condition|(
name|claims
operator|.
...
return|return;
block|}
comment|// Otherwise one of the aud claims must match the request URL
name|expectedAudience
operator|=
operator|(
name|String
