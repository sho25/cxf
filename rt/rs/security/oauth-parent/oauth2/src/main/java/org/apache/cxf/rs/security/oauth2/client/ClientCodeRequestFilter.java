...
expr_stmt|;
block|}
comment|// let the request continue if the token context is already available
return|return;
block|}
comment|// start the code flow
name|Response
name|codeResponse
init|=
...
if|if
condition|(
name|codeResponseQueryParamsAvailable
argument_list|(
name|requestParams
argument_list|)
operator|&&
...
comment|// unless the current request URI matches a non-null completeUri
if|if
condition|(
name|startUri
operator|==
...
return|;
block|}
comment|// If completeUri is null or startUri equals to it then check the code flow
comment|// response properties, if code parameters are set then it is the end of the flow
if|if
condition|(
name|completeUri
operator|==
...
specifier|public
name|void
name|setUseAuthorizationHeader
parameter_list|(
name|boolean
name|useAuthorizationHeader
parameter_list|)
