...
name|definitionsMap
decl_stmt|;
comment|/**      * The schemaCacheMap is used as a cache of SchemaInfo against the WSDLDefinitions.      * Note that the value is a WeakReference of ServiceSchemaInfo - Here the ServiceSchemaInfo in turn holds      * the key WSDL Definition. Since the value always strongly refers to the key, we make the value also      * weak - so that whenever their is no external strong reference to the key, the entry will qualify for      * removal from the map.      * https://issues.apache.org/jira/browse/CXF-1621      *      */
specifier|final
name|Map
argument_list|<
name|Definition
