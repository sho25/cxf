...
block|}
block|}
comment|/**      * For a schema, require all namespace with prefixes to be imported. In theory,      * if a namespace has a prefix but is not used, the import is not needed, and could      * cause a problem. In practice, the code generating these schemas should not be adding      * spurious prefixes. Hypothetically, we could check that, in the overall WSDL, that      * all of the schemas that we are importing are accounted for. However, that's a validation      * that is best left to the validator.      * @param schemaInfo Schema to process.      */
specifier|protected
name|void
name|addRequiredSchemaImports
parameter_list|(
name|SchemaInfo
name|schemaInfo
parameter_list|)
...
argument_list|()
decl_stmt|;
comment|// we need an import for every namespace except the main one.
name|String
...
expr_stmt|;
block|}
comment|// add imports for those schemata which are inlined.
name|addRequiredSchemaImports
argument_list|(
name|schemaInfo
argument_list|)
