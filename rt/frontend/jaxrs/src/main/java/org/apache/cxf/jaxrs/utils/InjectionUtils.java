...
block|}
block|}
comment|// it might be a TypeVariable, or a GenericArray.
return|return
literal|null
return|;
block|}
...
name|result
init|=
name|instantiateFromParameterHandler
argument_list|(
name|value
argument_list|,
name|pClass
...
parameter_list|)
block|{
comment|// TODO: Consider always checking custom parameter handlers first.
comment|// Right now, Locale and Date are two special cases so it's very cheap
comment|// just to check if it is Locale or not;
if|if
condition|(
name|Locale
operator|.
