...
specifier|final
name|String
name|EXPAND_QUERY_VALUE_AS_COLLECTION
init|=
literal|"expand.query.value.as.collection"
decl_stmt|;
specifier|private
name|String
name|scheme
...
parameter_list|()
block|{     }
comment|/**      * Creates builder with empty URI and properties      */
specifier|public
name|UriBuilderImpl
parameter_list|(
name|Map
...
argument_list|)
decl_stmt|;
comment|// Bad hack to bypass the TCK usage of bogus URI with empty paths containing matrix parameters,
comment|// which even URI class chokes upon; cheaper to do the following than try to challenge,
comment|// given that URI RFC mentions the possibility of empty paths, though no word on the possibility of
comment|// such empty paths having matrix parameters...
name|int
name|schemeIndex
init|=
name|pathEncoded
...
operator|&&
operator|!
name|schemeSpecificPartMatchesUriPath
argument_list|(
name|uri
argument_list|)
condition|)
...
argument_list|()
decl_stmt|;
comment|// Expand query parameter as "name=v1,v2,v3"
if|if
condition|(
name|isQuery
operator|&&
...
block|{
comment|// Expand query parameter as "name=v1&name=v2&name=v3", or use dedicated
comment|// separator for matrix parameters
for|for
control|(
name|Iterator
argument_list|<
...
parameter_list|)
block|{
comment|// This can be a start of replacing URI class Parser completely
comment|// but it can be too complicated, the following code is needed for now
comment|// to deal with URIs containing template variables.
name|int
name|index
init|=
name|uri
...
return|;
block|}
comment|//the clarified rules for encoding values of uri templates are:
comment|//  - encode each value contextually based on the URI component containing the template
comment|//  - in path templates, by default, encode also slashes (i.e. treat all path templates as
comment|//    part of a single path segment, to be consistent with @Path annotation templates)
comment|//  - for special cases when the slash encoding in path templates is not desired,
comment|//    users may use the newly added build methods to override the default behavior
annotation|@
name|Override
specifier|public
name|URI
