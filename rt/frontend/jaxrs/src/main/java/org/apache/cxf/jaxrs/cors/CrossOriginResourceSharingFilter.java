...
argument_list|)
decl_stmt|;
comment|/*          * CORS doesn't send enough information with a preflight to accurately identity the single method          * that will handle the request. CrossOriginResourceSharingPaths provides annotations by path/method          * for this case. If none of those apply, a plain class level CrossOrginResourceSharing is the          * best we can do.          */
name|String
name|requestUri
init|=
name|HttpUtils
...
condition|)
block|{
comment|/* search the path/method pair. */
for|for
control|(
name|CrossOriginResourceSharing
name|pathAnn
...
control|)
block|{
comment|/* A very simple path policy! If someone wants to turn this into                  * searching up the tree, they are welcome.                  */
if|if
condition|(
name|pathAnn
operator|.
