...
decl_stmt|;
comment|//TODO register streams using specific keys so that they can be individually unregistered
specifier|private
name|List
argument_list|<
name|Object
...
name|Path
argument_list|(
literal|"subscribe/alternate"
argument_list|)
specifier|public
name|StreamingOutput
name|getAlternateContinuousFeed
parameter_list|()
block|{
return|return
operator|new
...
name|WebApplicationException
block|{
comment|// return the last entry
name|out
operator|.
name|write
argument_list|(
name|convertEntryToHtmlFragment
argument_list|(
operator|(
operator|(
name|LinkedList
...
parameter_list|)
block|{
comment|//TODO use an executor to broadcast the record asynchronously
comment|//TODO take the search condition in consideration to filter out those non-matching entries
if|if
condition|(
name|activeStreams
operator|.
...
try|try
block|{
comment|// switch TCCL as abdera uses TCCL to load various abdera classes during serialization
name|Thread
operator|.
name|currentThread
argument_list|()
