...
argument_list|()
expr_stmt|;
comment|// handleNormalRequest(undertowExchange);
block|}
catch|catch
parameter_list|(
name|Exception
...
argument_list|)
expr_stmt|;
comment|/*              * if (AtmosphereUtils.useAtmosphere(request)) { try {              * framework.doCometSupport(AtmosphereRequestImpl.wrap(request),              * AtmosphereResponseImpl.wrap(response)); } catch (ServletException e) { throw new              * IOException(e); } return; } else { super.handleRequest(undertowExchange); }              */
try|try
block|{
name|framework
operator|.
...
name|Exception
block|{
comment|// if (AtmosphereUtils.useAtmosphere(request)) {
try|try
block|{
name|framework
operator|.
