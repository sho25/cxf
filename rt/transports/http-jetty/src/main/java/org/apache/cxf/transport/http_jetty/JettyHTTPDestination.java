...
condition|)
block|{
comment|//make sure the continuation is stripped down
name|JettyContinuationWrapper
name|c
init|=
name|p
...
argument_list|()
expr_stmt|;
comment|// now that we got the message we don't need ContinuationInfo
comment|// as we don't know how continuation was suspended, by jetty wrapper
comment|// or directly in which (latter) case we need to ensure that an original user object
comment|// if any, need to be restored
name|cont
operator|.
name|setObject
argument_list|(
...
name|message
init|=
literal|"No message for existing continuation, status : "
operator|+
operator|(
name|cont
operator|.
...
name|message
operator|+=
literal|", ContinuationInfo object is unavailable"
expr_stmt|;
block|}
name|LOG
operator|.
