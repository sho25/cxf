...
expr_stmt|;
block|}
comment|//if no jndiDestination or it fails see if jmsDestination is set and try to create it.
if|if
condition|(
name|requestDestination
operator|==
...
literal|null
decl_stmt|;
comment|//Reply Destination is used (if present) only if the session is point-to-point session
if|if
condition|(
name|isQueue
condition|)
