...
expr_stmt|;
block|}
comment|// Incase if the Content-Type header key is Content-Type replace with JMS_Content_Type
if|if
condition|(
name|entry
operator|.
...
argument_list|()
expr_stmt|;
comment|// throw new RuntimeException("No JMS_CLIENT_REQUEST_HEADERS set in message");
block|}
name|JMSUtils
operator|.
name|setMessageProperties
...
name|JMSException
block|{
comment|/*          * Can this be changed to producer.send(destination, jmsMessage, deliveryMode, priority, timeToLive);          */
if|if
condition|(
name|destination
operator|instanceof
