...
operator|new
name|PrincipalAuthorizationPolicy
argument_list|(
name|sc
argument_list|)
decl_stmt|;
...
argument_list|()
operator|||
name|isWSAddressingReplyToSpecified
argument_list|(
name|exchange
argument_list|)
operator|)
condition|)
block|{
comment|//For one-ways and WS-Addressing invocations with ReplyTo address,
...
condition|)
block|{
comment|//We don't need to cache it, but we may need to consume it in order for the client
comment|// to be able to receive a response. (could be blocked sending)
comment|//However, also don't want to consume indefinitely.   We'll limit to 16M.
try|try
block|{
name|IOUtils
operator|.
...
argument_list|()
expr_stmt|;
name|closeResponseOutputStream
argument_list|(
name|response
argument_list|)
expr_stmt|;
...
parameter_list|)
block|{
comment|// response.getWriter() has already been called
block|}
block|}
specifier|private
name|int
...
name|HTTP_OK
decl_stmt|;
comment|// put the code in the message so that others can get it
name|message
operator|.
name|put
argument_list|(
