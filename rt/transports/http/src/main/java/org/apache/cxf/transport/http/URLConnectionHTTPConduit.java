...
specifier|final
name|String
name|SET_REASON_PHRASE_NOT_NULL
init|=
literal|"set.reason.phrase.not.null"
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
try|try
block|{
comment|//                    cout = connection.getOutputStream();
if|if
condition|(
name|System
operator|.
...
condition|)
block|{
comment|//some http server like tomcat 8.5+ won't return the
comment|//reason phrase in response, return a informative value
comment|//to tell user no reason phrase in the response instead of null
return|return
