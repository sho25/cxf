...
argument_list|)
expr_stmt|;
comment|// ADDRESSING_ASSERTION is normalized, so check only the default namespace
if|if
condition|(
name|type
operator|.
...
name|warning
argument_list|(
literal|"Detected NONE value in ReplyTo WSA header for request-respone MEP"
argument_list|)
expr_stmt|;
block|}
else|else
...
condition|)
block|{
comment|// the in and out messages are on the same operation
comment|// and we were able to get a response for it.
name|message
operator|.
name|remove
argument_list|(
...
condition|)
block|{
comment|// if the decoupled endpoint context prop is set and the address is relative, return the absolute url.
specifier|final
name|String
name|replyTo
init|=
...
return|;
block|}
comment|/**      * Check for NONE ReplyTo value in request-response MEP      * @param message the current message      * @param maps the incoming MAPs      */
specifier|private
name|void
name|checkReplyTo
parameter_list|(
...
name|getString
argument_list|(
literal|"REPLYTO_NOT_SUPPORTED_MSG"
argument_list|)
argument_list|,
name|maps
operator|.
