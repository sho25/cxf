...
condition|)
block|{
comment|//fault occured trying to sent the message, remove it
name|uncorrelatedExchanges
operator|.
name|remove
argument_list|(
...
block|}
block|}
comment|/*         NodeList headerElements =             header.getElementsByTagNameNS(maps.getNamespaceURI(), "*");                 for (int i = 0; i< headerElements.getLength(); i++) {             Node headerElement = headerElements.item(i);             if (Names.WSA_NAMESPACE_NAME.equals(headerElement.getNamespaceURI())) {                 header.removeChild(headerElement);             }         } */
block|}
...
name|get
argument_list|(
literal|"deferred.fault.observer.notification"
argument_list|)
argument_list|)
operator|&&
name|endpoint
