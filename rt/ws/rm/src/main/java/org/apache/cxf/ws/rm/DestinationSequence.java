...
name|nextInOrder
decl_stmt|;
comment|//be careful, must be used in sync block
specifier|private
name|Map
argument_list|<
name|Long
...
condition|)
block|{
comment|// only save message, when policy verification is successful
comment|// otherwise msgs will be stored and redelivered which do not pass initial verification
comment|// as interceptor is called in a later phase than the capturing
name|PolicyVerificationInInterceptor
name|intercep
init|=
operator|new
...
argument_list|)
expr_stmt|;
comment|// in case no attachments are available, cos can be saved directly
if|if
condition|(
name|message
operator|.
...
block|}
block|}
comment|// this method is only used for redelivery
name|boolean
...
condition|)
block|{
comment|//next was found, don't resume everything, just the next one
name|c
operator|.
name|resume
argument_list|()
...
return|return;
block|}
comment|// only updating the sequence
name|store
operator|.
name|persistIncoming
argument_list|(
