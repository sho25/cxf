...
parameter_list|)
block|{
comment|// deprecated redelivery mode check
if|if
condition|(
name|MessageUtils
operator|.
...
argument_list|(
literal|"org.apache.cxf.ws.rm.destination.redeliver"
argument_list|)
argument_list|)
condition|)
block|{
...
name|warning
argument_list|(
literal|"Use RetryPolicy to enable the redelivery mode"
argument_list|)
expr_stmt|;
return|return
literal|true
...
block|{
comment|// in case message is not an application message, release SAVED_CONTENT
comment|// otherwise tmp files will not be closed
name|CachedOutputStream
name|cos
init|=
operator|(
...
condition|)
block|{
comment|// set this property to change the acknowledging behavior
name|message
operator|.
name|put
argument_list|(
