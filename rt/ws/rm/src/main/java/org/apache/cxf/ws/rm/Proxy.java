...
condition|)
block|{
comment|//for close messages, the server may be gone and nothing we can do so don't pollute the logs
name|LOG
operator|.
name|log
argument_list|(
...
name|WARNING
argument_list|,
literal|"Could not send CloseSequence message: "
operator|+
name|exception
operator|.
name|getCause
...
argument_list|)
decl_stmt|;
comment|// WS-RM requires ws-addressing
name|WSAddressingFeature
name|wsa
init|=
operator|new
