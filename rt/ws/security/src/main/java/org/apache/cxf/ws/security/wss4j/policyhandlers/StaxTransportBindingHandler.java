...
argument_list|)
expr_stmt|;
comment|/*TODO                else if (token instanceof IssuedToken) {                 SecurityToken secTok = getSecurityToken();                                  if (includeToken(token.getIncludeTokenType())) {                     //Add the token                     addEncryptedKeyElement(cloneElement(secTok.getToken()));                 }             } */
block|}
elseif|else
if|if
condition|(
...
name|Exception
block|{
comment|/* TODO if (token instanceof IssuedToken             || token instanceof SecureConversationToken             || token instanceof SecurityContextToken             || token instanceof SpnegoContextToken) {             addSig(doIssuedTokenSignature(token, wrapper));         } else */
if|if
condition|(
name|token
operator|instanceof
