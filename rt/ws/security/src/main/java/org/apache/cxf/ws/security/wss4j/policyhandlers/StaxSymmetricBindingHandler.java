...
argument_list|)
expr_stmt|;
comment|// TODO sig.setEncrKeySha1value(tok.getSHA1());
block|}
block|}
if|if
condition|(
...
comment|// tempTok.setSHA1(getSHA1(encrKey.getEncryptedEphemeralKey()));
name|getTokenStore
argument_list|()
operator|.
name|add
...
parameter_list|()
block|{
comment|// findEncryptedKeyToken(message);
comment|/*         SecurityEvent event = findEncryptedKeyEvent(message);         if (event != null) {             org.apache.xml.security.stax.securityToken.SecurityToken securityToken =                  ((EncryptedKeyTokenSecurityEvent)event).getSecurityToken();             if (securityToken != null) {                 Date created = new Date();                 Date expires = new Date();                 expires.setTime(created.getTime() + 300000);                                  String encryptedKeyID = securityToken.getId();                 System.out.println("ID: " + encryptedKeyID);                 SecurityToken tempTok = new SecurityToken(encryptedKeyID, created, expires);                 // tempTok.setSecret(securityToken.g);                 //tempTok.setSHA1(getSHA1((byte[])wser                 //                        .get(WSSecurityEngineResult.TAG_ENCRYPTED_EPHEMERAL_KEY)));                 getTokenStore().add(tempTok);                                  return encryptedKeyID;             }         }         System.out.println("EVENT NULL?: " + (event == null));         */
return|return
literal|null
return|;
block|}
comment|/*      * TODO     private SecurityToken findEncryptedKeyToken(Message message) {         @SuppressWarnings("unchecked")         final List<SecurityEvent> incomingEventList =              (List<SecurityEvent>) message.getExchange().get(SecurityEvent.class.getName() + ".in");         if (incomingEventList != null) {             for (SecurityEvent incomingEvent : incomingEventList) {                 if (WSSecurityEventConstants.EncryptedPart == incomingEvent.getSecurityEventType()                     || WSSecurityEventConstants.EncryptedElement                          == incomingEvent.getSecurityEventType()) {                     org.apache.xml.security.stax.securityToken.SecurityToken token =                          ((AbstractSecuredElementSecurityEvent)incomingEvent).getSecurityToken();                     if (token != null&& token.get) {                                              }                 }             }         }         return null;     }     */
specifier|private
name|String
name|getSHA1
parameter_list|(
