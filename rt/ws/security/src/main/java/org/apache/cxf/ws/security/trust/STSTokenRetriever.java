...
condition|)
block|{
comment|// If we have no delegation token then try to retrieve a cached token from the message
name|secToken
operator|=
name|tokenCacher
operator|.
...
block|{
comment|// Otherwise try to get a cached token corresponding to the delegation token
if|if
condition|(
name|onBehalfOfToken
operator|!=
...
condition|)
block|{
comment|// Check to see whether the token needs to be renewed
name|secToken
operator|=
name|renewToken
argument_list|(
...
init|=
name|getAddressingNamespaceURI
argument_list|(
name|message
argument_list|)
decl_stmt|;
