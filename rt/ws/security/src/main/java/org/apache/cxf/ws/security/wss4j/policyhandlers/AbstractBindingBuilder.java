...
comment|// WSS4J did not create during message signature creation.
name|part
operator|=
operator|new
name|WSEncryptionPart
...
comment|// the encrypted vector to prevent duplication / errors in encryption.
return|return
...
comment|// to process and assert them one at a time.  Additionally, a found list
comment|// should be applied to all operations that involve adding anything to
comment|// the signed vector to prevent duplication in the signature.
return|return
name|getPartsAndElements
argument_list|(
literal|true
...
comment|// elemenet with the same name and namespace as that in the
