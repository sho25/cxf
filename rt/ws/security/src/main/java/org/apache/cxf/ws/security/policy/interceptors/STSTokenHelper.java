...
begin_comment
comment|/*  * STSClient stsClient = new STSClient(bus); stsClient.setServiceQName(new QName(stsProps.get(STS_NAMESPACE),  * stsProps.get(STS_SERVICE_NAME))); Map<String, Object> props = new HashMap<String, Object>(); for  * (Map.Entry<String, String> entry : stsProps.entrySet()) { if  * (SecurityConstants.ALL_PROPERTIES.contains(entry.getKey())) { props.put(entry.getKey(),  * processFileURI(entry.getValue())); } } stsClient.setProperties(props);  * stsClient.setWsdlLocation(stsProps.get(STS_WSDL_LOCATION)); stsClient.setEndpointQName(new  * QName(stsProps.get(STS_NAMESPACE), stsProps.get(STS_ENDPOINT_NAME)));  * stsClient.setAllowRenewingAfterExpiry(true); stsClient.setEnableLifetime(true);  * stsClient.setTokenType(SAML2_TOKEN_TYPE); stsClient.setKeyType(BEARER_KEYTYPE); if (token != null) {  * stsClient.setActAs(token); } token =  * message.getContextualProperty(SecurityConstants.STS_TOKEN_ON_BEHALF_OF); if (token != null) {  * stsClient.setOnBehalfOf(token); } Object o =  * message.getContextualProperty(SecurityConstants.STS_APPLIES_TO); String appliesTo = null == o ? null :  * o.toString(); appliesTo = null == appliesTo ?  * message.getContextualProperty(Message.ENDPOINT_ADDRESS).toString() : appliesTo;  * stsClient.setMessage(message);  */
end_comment
