...
decl_stmt|;
comment|//
comment|// Verify the certificate chain associated with signature verification if
comment|// it exists. If it does not, then try to verify the (single) certificate
...
name|warning
argument_list|(
literal|"The certificate chain used for the signature is not trusted"
argument_list|)
expr_stmt|;
throw|throw
operator|new
...
condition|)
block|{
comment|//CXF-2150 - we need to check the UsernameTokens
for|for
control|(
name|WSSecurityEngineResult
name|o
...
name|warning
argument_list|(
literal|"Non-digest UsernameToken found, but digest required"
argument_list|)
expr_stmt|;
throw|throw
operator|new
