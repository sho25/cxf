...
argument_list|)
decl_stmt|;
comment|// REVISIT: Something is not setup quite right with this test.  In
comment|// the DSI Servant, we don't get the expected request operation name
comment|// that we set above.  Instead, we get a null value.  For now, just
comment|// add null to the operation map until we know how to fix the test.
name|map
operator|.
name|put
argument_list|(
