...
decl_stmt|;
comment|/**      * Determines how the classpath is applied to the executed command,      * either as a command line argument, or as an environment variable      * for the executing process. Only applicable when fork is set to<code>true</code>      * (which is always the case for JDK 9+).      *<p>      * Primarily useful on Windows because of length limitations for command arguments,      * which is why the default is<code>true</code> in that case.      *      * @parameter default-value="false"      * @since 3.3      */
specifier|private
name|Boolean
...
condition|)
block|{
comment|// Since JEP 261 ("Jigsaw"), access to some packages must be granted
name|fork
operator|=
literal|true
expr_stmt|;
name|boolean
name|skipXmlWsModule
init|=
name|JavaUtils
operator|.
...
condition|)
block|{
comment|// Windows does not allow for very long command lines,
comment|// so by default CLASSPATH environment variable is used.
name|classpathAsEnvVar
operator|=
literal|true
expr_stmt|;
...
name|append
argument_list|(
literal|"Try to run this goal using<fork>true</fork> and "
operator|+
literal|"<classpathAsEnvVar>true</classpathAsEnvVar>."
argument_list|)
expr_stmt|;
block|}
name|msg
