...
name|utils
operator|.
name|JAXRSServerFactoryCustomizationUtils
import|;
end_import
...
block|}
block|}
comment|// observing JAXRSCdiResourceExtension a "container" can customize that value to prevent some instances
comment|// to be added with the default qualifier
specifier|public
name|Collection
argument_list|<
name|String
argument_list|>
name|getExistingStandardClasses
parameter_list|()
block|{
return|return
name|existingStandardClasses
return|;
block|}
comment|/**      * Fires itsels, allows other extensions to modify this one.      * Typical example can be to modify existingStandardClasses to prevent CXF      * to own some beans it shouldn't create with default classifier.      *      * @param beforeBeanDiscovery the corresponding cdi event.      * @param beanManager the cdi bean manager.      */
name|void
name|onStartup
parameter_list|(
annotation|@
...
name|loadClass
argument_list|(
literal|"org.apache.webbeans.web.lifecycle.WebContainerLifecycle"
argument_list|)
expr_stmt|;
name|webHandled
operator|=
...
name|loadClass
argument_list|(
literal|"org.jboss.weld.module.web.WeldWebModule"
argument_list|)
expr_stmt|;
name|webHandled
operator|=
...
argument_list|)
expr_stmt|;
comment|// Adding the extension for dynamic providers registration and instantiation
if|if
condition|(
name|bus
operator|.
...
expr_stmt|;
block|}
comment|// always add the standard context classes
name|InjectionUtils
operator|.
name|STANDARD_CONTEXT_CLASSES
operator|.
...
argument_list|)
expr_stmt|;
comment|// add custom contexts
name|contextTypes
operator|.
name|addAll
argument_list|(
name|getCustomContextClasses
argument_list|()
argument_list|)
expr_stmt|;
comment|// register all of the context types
name|contextTypes
operator|.
name|forEach
argument_list|(
...
expr_stmt|;
block|}
comment|/**      * Registers created CreationalContext instances for disposal      */
specifier|public
name|void
name|registerCreationalContextForDisposal
parameter_list|(
annotation|@
name|Observes
specifier|final
...
condition|)
block|{
comment|// normal scoped beans will return us a proxy in getInstance so it is singletons for us,
comment|// @Singleton is indeed a singleton
comment|// @Dependent should be a request scoped instance but for backward compat we kept it a singleton
comment|//
comment|// other scopes are considered request scoped (for jaxrs)
comment|// and are created per request (getInstance/releaseInstance)
specifier|final
name|ResourceProvider
name|resourceProvider
decl_stmt|;
...
argument_list|)
expr_stmt|;
comment|// if not a singleton we manage it per request
comment|// if @Singleton the container handles it
comment|// so we only need this case here
if|if
condition|(
name|Dependent
operator|.
...
return|;
block|}
comment|// warn: several impls use @Dependent == request so we should probably add a flag
specifier|private
name|boolean
name|isConsideredSingleton
parameter_list|(
